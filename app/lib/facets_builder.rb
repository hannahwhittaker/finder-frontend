...
private
  ...
  attr_reader :content_item, :search_results, :facet_hashes, :value_hash
  ...
  def allowed_values(facet_hash)
    ...
    if registries.all.has_key?(facet_key)
      facet_hash['allowed_values'] = allowed_values_from_registry(facet_key)
    elsif (facet_details = search_results.dig("facets", facet_key))
      facet_hash['allowed_values'] = allowed_values_for_facet_details(facet_key, facet_details)
    end
  end
